...
//        }
            ...
            if ($this->id > 0) {
                ...
                if ($resultado == 0) {
                    ...
                    while ($obj = mysql_fetch_object($con)) {
                        ...
                        $arrfieldsnocomma = array('s24_cliente_cli_id' => $this->idcli, 'usr_dtcreate' => $this->UTILITY->date_now_server(), 'usr_habilitado' => $this->habilitado);
    ...
    private function usrprfget() {
        ...
        $q = "SELECT * FROM s24_usuario_has_s24_perfiles WHERE s24_usuario_usr_id = $this->id ORDER BY s24_perfiles_prf_id ASC";
        $con = mysql_query($q, $this->conexion) or die(mysql_error() . "***ERROR: " . $q);
        $arrassigned = array();
        $arravailable = array();
        while ($obj = mysql_fetch_object($con)) {
            $arrassigned[] = array('id' => $obj->s24_perfiles_prf_id);
        }
        //se consultan los perfiles disponibles
        $q = "SELECT * FROM s24_perfiles ORDER BY prf_nombre ASC";
        $con = mysql_query($q, $this->conexion) or die(mysql_error() . "***ERROR: " . $q);
        while ($obj = mysql_fetch_object($con)) {
            $arravailable[] = array(
                'id' => $obj->prf_id,
    ...
    private function usrprfsave() {
        if ($this->id > 0) {
            //actualiza la informacion
            $q = "DELETE FROM s24_usuario_has_s24_perfiles WHERE s24_usuario_usr_id = " . $this->id;
            mysql_query($q, $this->conexion) or die(mysql_error() . "***ERROR: " . $q);
            $arrchk = explode('-', $this->chk);
            for ($i = 0; $i < count($arrchk); $i++) {
                $prf_id = intval($arrchk[$i]);
                if ($prf_id > 0) {
                    $q = "INSERT INTO s24_usuario_has_s24_perfiles (s24_usuario_usr_id, s24_perfiles_prf_id, dtcreate) VALUES ($this->id, $prf_id, " . $this->UTILITY->date_now_server() . ")";
                    mysql_query($q, $this->conexion) or die(mysql_error() . "***ERROR: " . $q);
                }
            }
            $arrjson = array('output' => array('valid' => true, 'id' => $this->id));
